...
class RoomStore extends ChangeNotifier {
  ...
  Future<void> checkInWithStayMode(Room room, String stayMode) async {
    await _runAction(room.roomId, 'check_in', () async {
      await _backend.checkIn(room.roomId, stayMode: stayMode);
    });
  }
  ...
  Future<void> extendStay(
    ...
    await _runAction(room.roomId, 'extend_stay', () async {
      await _backend.extendStay(
        room.roomId,
        stayMode: stayMode,
  ...
  Future<void> receivePayment(
    ...
    await _runAction(room.roomId, 'receive_payment', () async {
      await _backend.receivePayment(
        room.roomId,
        amount: amount,
        method: method,
        reference: reference,
